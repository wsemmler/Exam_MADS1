...
def preprocess(texts, nlp, stop_words):
    cleaned_texts = []
    for i, doc in enumerate(tqdm(nlp.pipe(texts, batch_size=100))):
...
corpus_path = Path("./results")
with open(corpus_path / "corpus_texts.txt", "w", encoding="utf-8") as f:
    for doc in corpus:
        f.write(doc + "\n")

with open(corpus_path / "corpus_texts.txt", "r", encoding="utf-8") as f:
    lines = f.readlines()
...
sns.despine()
fig.tight_layout()
fig.savefig("raw_text.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close()
...
sns.despine()
fig.tight_layout()
fig.savefig("cleaned_text.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close()
...
topics_line = ", ".join([f"Topic {topic_id + 1}: {prob:.4f}" for topic_id, prob in topics[:10]])
...
for doc_id in doc_ids:
    topics = get_doc_topic_by_id(doc_id, jsonl_path, model10)
    topics_line = ", ".join([f"Topic {tid+1}: {p:.4f}" for tid, p in topics[:10]])
    print(f"Doc {doc_id} top topics → {topics_line}")
...
#     model = LdaModel.load(f"models/lda_{ntopics}_model")
#     eval_lda_model(ntopics, model, gensim_corpus, [doc["tokens"] for doc in cleaned_texts])

# text = "Apple is releasing a new iPhone model next week!!! It costs $999 and ships in 2 days."
# doc = nlp(text)
# tokens = clean_doc(doc, stop_words)

# print("Original text:", text)
# print("Cleaned doc:", tokens)
